 ...
 *            触摸事件将继续传递到控件上
 */
static bool onhttpActivityTouchEvent(const MotionEvent &ev) {
...
static bool onButtonClick_ButtonHttpDownload(ZKButton *pButton) {
    ...
    //发起HTTP GET请求
    RestClient::Response r = RestClient::download("http://www.baidu.com/img/bd_logo1.png", save_path);
    if (r.code == 200) {
        //将下载的图片设置为背景图
        mTextview1Ptr->setBackgroundPic(save_path.c_str());
    } else {
      //下载出错
      mTextview1Ptr->setText(r.code);
    }
    return false;
}
